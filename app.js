/**
 * FinOfficer Automation Landing Page & Auto 24h Cache Engine
 */
...
async function loadProjectData() {
  const LOCAL_CACHE_KEY = 'fin_officer_github_daily_cache_v1';
...
function initCytoscapeGraph() {
  ...
  const elements = [];
  Object.values(appState.projects).forEach(p => {
    elements.push({ data: { id: p.id, label: p.id, color: '#10b981' } });
  });
  Object.values(appState.projects).forEach(p => {
